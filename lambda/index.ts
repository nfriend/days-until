...
export const languageStrings = {
    'en': {
        translation: {
            ...
            HELP_MESSAGE: `Sure.  You can create a new countdown by saying `
            ...
            EVENT_PROMPT: 'Would you like to create a new countdown or check an existing one?',
...
export const alexaHandlers = {
    ...
    'StartCountdownIntent': function () {
        ...
        if (this.event.request.dialogState !== 'COMPLETED') {
            ...
            const eventName = capitalize.words(this.event.request.intent.slots.CountdownEvent.value);
            ...
            if (/^\d{4}-W\d{1,2}-WE$/.test(eventDate)) {
                ...
                eventDate = moment(eventDate.replace(/-WE$/, '')).add(5, 'days').format('YYYY-MM-DD');
            ...
            if (!eventInfo) {
                ...
                if (dayDiff > 0) {
                    ...
                    if (dayDiff === -1) {
                        ...
                        this.emit(
                            ':tellWithCard', 
                            this.t('PAST_MESSAGE_1') + ` ${ -1 * dayDiff } ` + this.t('DAYS_AGO'),
                            this.t('SKILL_NAME'),
